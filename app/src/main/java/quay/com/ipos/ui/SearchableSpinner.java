...
public class SearchableSpinner extends Spinner implements View.OnTouchListener,
    ...
    private CustomAdapterAddress _arrayAdapter;
    ...
    private void init() {
        ...
        _arrayAdapter = (CustomAdapterAddress) getAdapter();
        if (!TextUtils.isEmpty(_strHintText)) {
            CustomAdapterAddress  customAdapterAddress = new CustomAdapterAddress(getContext(), R.layout.spinner_item_pss,R.id.text1,_items);

          //  ArrayAdapter arrayAdapter = new ArrayAdapter(_context, android.R.layout.simple_list_item_1, new String[]{_strHintText});
            _isFromInit = true;
            setAdapter(customAdapterAddress);
        }
    }
    ...
    public void setAdapter(SpinnerAdapter adapter) {
        ...
        if (!_isFromInit) {
            _arrayAdapter = (CustomAdapterAddress) adapter;
            if (!TextUtils.isEmpty(_strHintText) && !_isDirty) {
                CustomAdapterAddress  customAdapterAddress = new CustomAdapterAddress(getContext(), R.layout.spinner_item_pss,R.id.text1,_items);
             //   ArrayAdapter arrayAdapter = new ArrayAdapter(_context, android.R.layout.simple_list_item_1, new String[]{_strHintText});
                super.setAdapter(customAdapterAddress);
            } else {
