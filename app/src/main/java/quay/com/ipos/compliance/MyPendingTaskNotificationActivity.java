...
public class MyPendingTaskNotificationActivity extends AppCompatActivity implements
        ComplianceDetailHandler {
    private static final String TAG = MyPendingTaskNotificationActivity.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            this.activity = MyPendingTaskNotificationActivity.this;
        ActivityComplianceDetailBinding binding = DataBindingUtil.setContentView(this, R.layout.activity_compliance_detail);
        viewModel = new ComplianceViewModel(MyPendingTaskNotificationActivity.this);
...
//        Toast.makeText(activity, "On New Intent Called", Toast.LENGTH_SHORT).show();
    ...
    private void loadSubTaskList() {
        try {
            if (task_id != 0) {
                IPOSApplication.getDatabase().subtaskDao().getAllSubTaskofTask(task_id).observe(MyPendingTaskNotificationActivity.this, new Observer<List<SubTask>>() {
                    @Override
...
*/
        ...
        protected void onPostExecute(Boolean aVoid) {
           ...
           /* Intent intent = new Intent(getApplicationContext(),DashboardActivity.class);
            ...
            Log.i("data", aVoid + "");
            if (aVoid) {
                IPOSApplication.getDatabase().taskDao().getTaskById(task_id).observe(MyPendingTaskNotificationActivity.this, new Observer<Task>() {
                    @Override
                    public void onChanged(@Nullable Task task) {
                        Gson gson = new Gson();
                        Log.v(TAG, "taskData" + gson.toJson(task));
